# Name:    BuildGeoNETdll
# Purpose: Compile the mdGeoNET.dll wrapper
...
Write-Host "`n======================= WELCOME TO MELISSA GEOCODER OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdGeoNET.dll -platform:AnyCPU -target:library mdGeo_cSharpCode.cs" > $null

if (Test-Path ("mdGeoNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdGeoNET.dll"

  Write-Host "`nmdGeoNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdGeoNET.dll was not created successfully`n"
